...
<html>
<head>
	<title>THIRD FIRST TERM EXAMINATION REPORT SHEET</title>
...
<?php
		...
		if($exam_result){
			...
			$total_third_term_scores=0;// INITIALIZE OVERALL TOTAL SCORE FOR SECOND TERM SCORE TO ZERO
			...
			$third_term_divisor=0;	// INITIALIZE DIVIDER FOR  SECOND TERM SCORE TO ZERO
			...
			foreach ($exam_result as $result) {
				...
				elseif(count($scores_second_term)==2){
					...
					elseif($scores_second_term[0]->TERM!=3){
						$first_term=$scores_second_term[0]->TOTAL;
						$second_term==$scores_second_term[1]->TOTAL;
						$third_term="--";
					...
					if($scores_second_term[0]->TERM==1 ){
						$first_term=$scores_second_term[0]->TOTAL;
						$second_term="<li class='fa fa-minus'></i>";
						$third_term="<li class='fa fa-minus'></i>";
					...
					elseif($scores_second_term[0]->TERM==2){
						...
						$first_term="<li class='fa fa-minus'></i>";
						$second_term="<li class='fa fa-minus'></i>";
						$third_term=$scores_second_term[0]->TOTAL;
						$total_third_term_scores+=$scores_second_term[0]->TOTAL;
						$third_term_divisor+=1;
					}
				}				
				//END 
				...
				echo'
					...
					<tr>
						...
						<td>'.$result->EXAM.'</td>
						<td>'.$total.'</td>
						<td>'.round($this->student_model->fetch_exam_average($subject_data),1).'</td>
						<td>'.$this->student_model->fetch_exam_highest($subject_data).'</td>
						<td>'.$first_term.'</td>
						<td>'.$second_term.'</td>
							<td>'.$third_term.'</td>
						<td>'.$grade[0].'</td>
			...
			<thead><tr><th>1<sup>ST</sup> TERM AVG</th><th>2<sup>ND</sup> TERM AVG</th><th>3<sup>RD</sup> TERM AVG</th><th>OVERALL AVG</th></tr></thead>
			<tbody>
			<tr>
			...
			<td>'.number_format($total_third_term_scores/$third_term_divisor,2).'</td>
			<td>'.number_format(($total_first_term_scores+$total_second_term_scores+$total_third_term_scores)/($first_term_divisor+$second_term_divisor+$third_term_divisor),2).'</td></tr>
			</tbody>
			</table>
			</div>
			</div>
